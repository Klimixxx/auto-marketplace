...
function buildAdditionalEyebrow(listing, tradeTypeInfo = null) {
  ...
  if (assetType && typeof assetType === "string") {
    ...
    if (normalized && normalized !== tradeTypeInfo?.label)
...
function tradeTypeLabel(type) {
  ...
  if (!text) return null;
  return (
    localizeListingBadge(text) ||
    translateValueByKey("asset_type", text) ||
    text
  );
}
...
function collectTypeStrings(source, out) {
  ...
  if (typeof source === "string" || typeof source === "number") {
    ...
    const stripped = text
      .replace(/^(?:\s*(?:тип|вид)\s+(?:торг|процед)\s*(?:[:–—-]\s*)?)/i, "")
      .trim();
  ...
  if (typeof source === "object") {
    const labelMatches = TYPE_LABEL_FIELDS.some(
      (field) =>
        ...
        TYPE_LABEL_PATTERN.test(source[field])
    );
    ...
    for (const [key, value] of Object.entries(source)) {
      ...
      if (Array.isArray(value) || (value && typeof value === "object")) {
        ...
        !TYPE_LABEL_FIELDS.includes(key)
      ) {
        collectTypeStrings(value, out);
      }
    }
...
function resolveTradeType(listing) {
  ...
  const backendLabel =
    listing?.trade_type_label || listing?.resolved_trade_type_label || null;
  const backendRaw =
    listing?.trade_type_resolved ??
    listing?.resolved_trade_type ??
    listing?.normalized_trade_type ??
    listing?.trade_type;
  ...
  const hasPublic = lowers.some(
    (text) =>
      text.includes("публич") ||
      text.includes("public") ||
      text.includes("предлож") ||
      text.includes("offer")
  );
  const hasAuction = lowers.some(
    (text) => text.includes("аукцион") || text.includes("auction")
  );
  const hasOpen = lowers.some(
    (text) => text.includes("открыт") || text.includes("open")
  );
    ...
    if (kind === "public_offer") {
      label =
        normalized.find((text) => {
          ...
          return (
            lower.includes("публич") ||
            lower.includes("предлож") ||
            lower.includes("offer")
          );
        }) || tradeTypeLabel("public_offer");
    } else if (kind === "open_auction") {
      const auctionLabel = normalized.find((text) =>
        /аукцион|auction/i.test(text)
      );
      if (auctionLabel) {
        ...
        if (lower.includes("открыт") || lower.includes("open")) {
          ...
          label = hasOpen
            ? tradeTypeLabel("open_auction")
            : tradeTypeLabel("auction");
        }
      } else {
        label = hasOpen
          ? tradeTypeLabel("open_auction")
...
function classifyStatus(name) {
  ...
  if (lower.includes("заверш"))
    return { label: "Торги завершены", color: "#dc2626" };
  if (/(открыт|открыта|при[её]м|заявок)/i.test(lower))
    return { label: "Открыт прием заявок", color: "#16a34a" };
...
export default function ListingCard({
  ...
  const startPriceRaw =
    l.start_price ??
    pickDetailValue(l, ["start_price", "startPrice", "initial_price"]);
  ...
  const locationLabel = [l.city, region]
    ...
    .join(", ");
  const rawType =
    l.trade_type_resolved ??
    l.trade_type ??
    pickDetailValue(l, ["trade_type", "type"]);
  const tradeType =
    tradeTypeInfo?.label ||
    l.trade_type_label ||
    tradeTypeLabel(rawType) ||
    "Лот";
  const fallbackYear = pickDetailValue(l, [
    "year",
    "production_year",
    "manufacture_year",
    "year_of_issue",
  ...
  const additionalEyebrow = listingKind
    ? buildAdditionalEyebrow(l, tradeTypeInfo)
    : null;
      ...
      >
        ...
        >
          ...
          {onFav ? (
            ...
            >
              <span style={{color: fav && "#f59e0b"}} aria-hidden="true">{fav ? "★" : "☆"}</span>
                ...
                >
                  <span
                    style={{
                      ...
                      height: 8,
                      borderRadius: 999,
                      background: statusInfo.color || "#64748b",
      ...
      >
        ...
        >
          ...
          {onFav ? (
            ...
            >
              <span aria-hidden="true" style={{color: fav && "#f59e0b"}}>{fav ? "★" : "☆"}</span>
              <span>{fav ? "В избранном" : "В избранное"}</span>
            </button>
          ) : null}
        </div>
        ...
        <div style={{ display: "grid", alignContent: "start", gap: 8 }}>
          ...
          <div style={{ fontSize: 13, color: "#64748b" }}>
            {l.number ? (
              <span style={{ marginRight: 8 }}>№{l.number}</span>
            ) : null}
            {l.lot_number ? (
              <span style={{ marginRight: 8 }}>Лот №{l.lot_number}</span>
            ) : null}
          </div>

          {shortDescription ? (
            <div
              style={{ fontSize: 13, color: "#94a3b8", whiteSpace: "pre-line" }}
            >
              {shortDescription}
            </div>
          ) : null}
          ...
          {statusInfo ? (
            ...
            >
              ...
              />
              <span>
                Статус лота: <b>{statusInfo.label}</b>
              </span>
            </div>
          ) : null}
        </div>
