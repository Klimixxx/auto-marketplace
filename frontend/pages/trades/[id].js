...
export default function ListingPage({ item }) {
  ...
  return (
    <div className="container detail-page">
      ...
      <div className="detail-hero" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', alignItems: 'start' }}>
        ...
        <div className="detail-hero__info">
          ...
          <div className="detail-summary-card detail-summary-card--inline">
            ...
            {(statusDisplay || currentStageLabel || stageEndLabel || applicationDeadlineLabel || finishLabel) && (
              ...
              >
                {statusDisplay ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#000000 !important' }}>
                    {statusDisplay.color ? (
                      <span style={{ width: 10, height: 10, borderRadius: 999, background: statusDisplay.color, display: 'inline-block' }} />
                    ) : null}
                    <div style={{ color: '#000000 !important' }}><strong>Статус:</strong> {statusDisplay.label}</div>
                  </div>
                ) : null}
                {currentStageLabel ? <div style={{ color: '#000000 !important' }}><strong>Текущий этап:</strong> {currentStageLabel}</div> : null}
                {stageEndLabel ? <div style={{ color: '#000000 !important' }}><strong>Окончание этапа:</strong> {stageEndLabel}</div> : null}
                {applicationDeadlineLabel ? <div style={{ color: '#000000 !important' }}><strong>Приём заявок до:</strong> {applicationDeadlineLabel}</div> : null}
                {!stageEndLabel && !applicationDeadlineLabel && finishLabel ? (
                  <div style={{ color: '#000000 !important' }}><strong>Дата окончания:</strong> {finishLabel}</div>
                ) : null}
                {finishLabel && (stageEndLabel || applicationDeadlineLabel) && finishLabel !== stageEndLabel && finishLabel !== applicationDeadlineLabel ? (
                  <div style={{ color: '#000000 !important' }}><strong>Завершение торгов:</strong> {finishLabel}</div>
                ) : null}
              </div>
            )}
            ...
            <div className="detail-summary__actions">
              ...
              <button onClick={handleInspectionClick} className="button button-outline">Заказать осмотр</button>
              {item?.source_url ? (
