...
export default function AdminInspectionDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!token) { router.replace('/login?next=/admin/inspections'); return; }
      try {
  ...
  async function updateOrderStatus(nextStatus) {
    ...
    try {
      ...
      const data = await res.json();
      setItem((prev) => ({ ...(prev || {}), ...data, admin_unread: false, status: data.status }));
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new Event('admin-inspections-refresh'));
      }
  ...
  async function uploadPdf(e) {
    ...
    try {
      ...
      const data = await res.json();
      setItem((prev) => ({ ...(prev || {}), ...(data?.order || {}), admin_unread: false }));
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new Event('admin-inspections-refresh'));
      }
      alert('PDF загружен');
