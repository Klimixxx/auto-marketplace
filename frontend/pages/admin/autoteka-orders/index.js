...
export default function AdminAutotekaList() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!token) { location.href = '/login?next=/admin/autoteka-orders'; return; }
      try {
  ...
  useEffect(() => {
    ...
    async function loadStatuses() {
      try {
        const token = localStorage.getItem('token');
        if (!token) return;
        const res = await fetch(resolveApiUrl('/api/admin/autoteka-orders/statuses'), {
          headers: { Authorization: 'Bearer ' + token },
        });
        ...
        console.error('Failed to load autoteka statuses', err);
  ...
  useEffect(() => {
    ...
    async function loadOrders() {
      try {
        ...
        const url = resolveApiUrl(`/api/admin/autoteka-orders${params.toString() ? `?${params}` : ''}`);
        ...
        console.error('Failed to load autoteka orders', err);
        if (!ignore) {
          setError('Не удалось загрузить заказы Автотеки. Попробуйте позже.');
          setItems([]);
        }
      } finally {
  ...
  return (
    <AdminLayout me={me} title="Автотека">
      ...
      {!loading && !error && items.length === 0 && (
        <div>Заказы Автотеки не найдены.</div>
      )}
...
const rowUnread = { background: 'rgba(99,102,241,0.12)' };
const dot = { color: '#4338ca', fontSize: 18, lineHeight: 1 };
