...
export default function AutotekaOrders() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!token) { window.location.href = '/login?next=/autoteka'; return; }
      try {
        const res = await apiFetch('/api/autoteka/statuses');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiFetch(`/api/autoteka/me?${params.toString()}`);
        if (res.status === 401) { window.location.href = '/login?next=/autoteka'; return; }
        if (!res.ok) throw new Error('status ' + res.status);
