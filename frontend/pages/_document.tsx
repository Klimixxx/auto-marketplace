...
export default class MyDocument extends Document {
  render() {
    return (
      <Html lang="ru">
        <Head>
          {/* Preview Bridge: разрешает портфолио управлять автоскроллом */}
          <script
...
(function () {
  ...
  function step(now){
    ...
    if (next >= maxY - 1) { window.scrollTo({ top: 0, behavior: "instant" }); last = now; }
    raf = requestAnimationFrame(step);
  }
  function startAuto(s){ stopAuto(); auto = true; speed = +s > 0 ? +s : 30; last = 0; raf = requestAnimationFrame(step); }
